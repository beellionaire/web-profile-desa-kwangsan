...
<div class="container mx-auto p-4">
    <!-- Page Title -->
    <h1 class="text-3xl font-bold text-black mb-6">Tambah Berita</h1>
    ...
    <form action="{{ route('admin.berita.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="p-2">
            ...
            <input type="text" name="judul" placeholder="Judul Berita" value="{{ old('judul') }}"
        ...
        <div class="p-2">
            ...
            <textarea name="deskripsi" rows="6" placeholder="Tulis isi berita..."
                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#8c0327] focus:ring-[#8c0327] p-2 bg-[#f6f6f6]">{{ old('deskripsi') }}</textarea>
            @error('deskripsi')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="p-2">
            ...
            <input type="text" name="author" placeholder="Nama Penulis" value="{{ old('author') }}"
                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#8c0327] focus:ring-[#8c0327] p-2 bg-[#f6f6f6]">
            @error('author')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
